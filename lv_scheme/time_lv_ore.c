...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < nbits_len; i++) {
    ...
    byte_len_of_ctxt = element_length_in_bytes(ctxt->bit_ctxt[0])*NBITS[i] + element_length_in_bytes(ctxt->g1r1k21);
    // byte_len_of_ctxt =  element_length_in_bytes_compressed(ctxt->bit_ctxt[0])*NBITS[i] + element_length_in_bytes_compressed(ctxt->g1r1k21);
    ...
    byte_len_of_token = element_length_in_bytes(token->token_bit[0]->add_one) * NBITS[i] + element_length_in_bytes(token->token_bit[0]->minus_one) * NBITS[i] + element_length_in_bytes(token->g2r2k22);
    // byte_len_of_token = sizeof(bool) + sizeof(ore_params) + element_length_in_bytes_compressed(token->token_bit[0]->add_one) * NBITS[i] + element_length_in_bytes_compressed(token->token_bit[0]->minus_one) * NBITS[i] + element_length_in_bytes_compressed(token->g2r2k22);
